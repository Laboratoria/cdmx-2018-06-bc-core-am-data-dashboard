let sede = document.getElementById('buttons-campus'); // llamamos los id's del html para manipularlos con javascript (aparecer y desaparecer elementos) 
...
const sedes = (laboratoria) => {
  ...
  for (key in laboratoria) {
    ...
    aElement.id = key;
    aElement.innerHTML = key.toUpperCase();// imprimimos la variable 'key' en nuestro en nuestro elemento 'a' vinculando la data con los botones dinámicos
  ...
  for (key in laboratoria) {
    if (key === selectSede) {
      ...
      generation1.forEach(elements => { 
        ...
        seccion.addEventListener('click', () => dentroGen(laboratoria, selectSede)); // Pasamos la variable selectSede como parámetro para reutilizarla en nuestra siguiente función 
      });
    }
  }
};
const dentroGen = (laboratoria, selectSede) => { // 'dentroGen' significa "dentro de la generación seleccionada".
  countAverageContainer.style.display = 'block';
  gen.style.display = 'none';

  const sedeActual = selectSede;
  const selectGeneration = event.target.id; // En esta variable especificamos que el evento anterior se dotona si el id = a 'generation1'
  ...
  generation1.forEach(element => { 
    let average2 = 0; // Esta variable indica que antes cada iteración la suma de 'average2' vuleve a valer cero
    ...
    if (generation1 === selectGeneration) {
      ...
      for (student in students1) {
        ...
        if (completedPercentage1 < 60) { // Creamos una condicional para segmentar a las estudiantes por status de su porcentaje completado y a su vez imrpimimos en el DOM
      ...
      seccion.appendChild(mainAverageContainer); // seccion es padre de MainAverageContiner
