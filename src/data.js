...
window.computeStudentsStats = laboratoria => {
  ...
  for (city in laboratoria) {
    // primer iteración con for in
    let sede = city;
    const generations = Object.keys(laboratoria[city].generacion);
    // console.log(generations);
    generations.forEach(genNumber => {
      // 1º for each para extraer los valores de cada generación
      numberG = genNumber;
      ...
      students.forEach(student => {
        ...
        // console.log(topics);
        for (topic of topics) {
          let topicsRoute = Object.defineProperty(
            student.progreso.temas,
            topic,
            { writable: true }
          );
          ...
          for (i = 0; i < studentProgressTopic.length; i++) {
            ...
            studentProgressTopic[i].completedPercentage =
              studentProgressTopic[i].porcentajeCompletado;
            let topicProgress =
              (studentProgressTopic[i].duracionTemaCompletado * 100) /
              studentProgressTopic[i].duracionTema;
            studentProgressTopic[i].percentageDuration = Math.round(
              topicProgress
            );
            studentProgressTopic[i].subtopics =
              studentProgressTopic[i].subtemas;
          }
        }
...
window.computeGenerationsStats = laboratoria => {
  ...
  for (sedes in laboratoria) {
    ...
    for (let numberG of gen) {
      ...
      //declarar la variable con el totalde estudiantes dentro del .json
      let totalStud = stud.length;
      count = totalStud;
      for (eachStudent in stud) {
        eachGenPercent +=
          laboratoria[sedes].generacion[numberG].estudiantes[eachStudent]
